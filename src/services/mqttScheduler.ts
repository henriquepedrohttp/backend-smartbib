...
function processAutoCancel(): void {
  ...
  const rows = db.exec(
    `SELECT id, sala_id FROM reservas
     WHERE status = 'pendente'
       AND (
         (mqtt_inicio_enviado = 1 AND mqtt_inicio_enviado_at IS NOT NULL
          AND datetime(mqtt_inicio_enviado_at, '+10 minutes') <= datetime('now', 'localtime'))
         OR
         (mqtt_inicio_enviado = 0
  ...
  for (const row of rows[0].values) {
    ...
    console.log(`[MQTTScheduler] Reserva ${reservaId}: auto-cancelada (não confirmada em 10 min)`);
  }
